...
│   ├── atomic_write.py          # temp→rename 원자적 쓰기 (UTF-8 고정)
│   └── query_norm.py            # 회상 키 단일 정규화기 (emit·bootstrap 공용)
│
...
│
├── tests/test_factory.py        # 팩토리 자기테스트 (134 tests)
...
idoforgod의 advisory prose와 달리, CYS의 규칙은 **정적으로 강제**됩니다. `validate_harness.py`(49 머신체크 코드)가 위반 시 빌드를 실패시키고, 런타임 규칙은 hook으로 *발화*합니다. 설치 모드(`self-contained`/`in-project`)는 `.harness/GENOME.json`의 `install_mode`로 자동 분기됩니다.
...
| `TIER_OVERSPEND` | 순수검색 역할(gather/extract/format/qa-scan)에 opus 금지(역할→티어; synthesis/judge/critic/architecture는 opus 허용) | error(override 사유 시 warn) |
...
| `MEASUREMENT_DRIFT` | 정직성 — evals verdict가 존재하고 그중 `CYS-WINS`가 없는데 doc이 `CYS-WINS` 광고 시 차단(verdict 0개면 통과) | error |
...
| `context_guard.py` (`--mode=restore`) / `save_context.py` | SessionStart / PreCompact·SessionEnd | 컨텍스트 복원 / 스냅샷 저장 |
...
| 도메인 | n | 결과 |
|--------|---|------|
| deep-research | 5 | **+12.5pp `INCONCLUSIVE`** (CYS 우세, 마진 미달) — `examples/deep-research/evals/deep-research.verdict.json` |

> **stamp된 수치 데이터포인트는 deep-research 하나**(위 verdict 아티팩트). verification-heavy 레인은 **rate-limit으로 미완**(`_workspace/h2h/runs.json`: `n_actual:0`)이라 수치를 stamp하지 않습니다 — 미측정 데이터를 결과로 주장하지 않습니다(+37.5pp 교훈).
> **정직한 결론:** 현대 opus single-pass가 객관적 scorecard에서 이미 천장에 가까워 하네스가 **마진으로 이기는 것은 어렵습니다**. 더 많은 도메인을 뒤져 ≥15pp를 낚는 것은 벤치마크 게이밍이므로 하지 않습니다. 하네스의 가치는 scorecard 승리가 아니라 **parity(목표 충족) + 결정론적 인프라**(DNA 발화·비용 거버넌스·머신체크 계약)에 있습니다.
...
- **134 factory tests green · 4 예제 validate 0/0**
