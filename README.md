...
│
├── tests/test_factory.py        # 팩토리 자기테스트 (120 tests)
...
python3 -m pytest tests/test_factory.py -q                                    # 120 factory tests
...
- **3층위 장기기억 self-hosting** — 회상이 prose가 아니라 실행 단계로 배선(`MEMORY_RECALL_WIRED`)되고 에이전트가 회상을 실제 소비(`AGENT_MEMORY_CONTRACT`); 팩토리도 자기 `.harness/memory/` 빌드 기억을 dogfood
- **120 factory tests green · 4 예제 validate 0/0**
