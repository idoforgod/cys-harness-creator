...
- **L1 / L1.5 / L2는 fire-on-presence** — 각 validator(`validate_verification.py` / `validate_pacs.py` / `validate_review.py`)는 자기 로그 파일(`verification-logs/step-N-verify.md` / `pacs-logs/step-N-pacs.md` / `review-logs/step-N-review.md`)이 존재할 때만, `gate_or_block.py`를 통해 발화한다. 로그가 만들어졌으면 품질을 강제하고, 안 만들어졌으면 절대 false-block하지 않는다.
...
- §5의 "산출물 누락은 통과 불가"라는 anti-skip 바닥선은 **L0**(hook 내장 `l0_block`)이 강제한다 — 어떤 로그도 없이 스텝을 건너뛰는 것을 디스크 존재·크기 체크로 막는다.
