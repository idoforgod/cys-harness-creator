...
>
> **현 상태: 116 factory tests green, 4 예제 validate 0/0, idoforgod 8 use case 전부 conform, in-project 오버레이 설치·lift 배선·h2h 보강 완료 + 7-dim 적대적 전수감사(18 confirmed) 후 P0×3·robustness×4·P1×4·P2(A/B/C/G) 보강 완료 + 자동 교정 루프(린터+맞춤법+프리커밋, 팩토리 자기적용·게놈 전수) + 3층위 장기기억 self-hosting(회상 배선 + 에이전트 소비 계약) 추가.**
...
- **후속(미구현, 정직 기록)**: P2(단계간 `stage-N-facts.jsonl` 릴레이 + SOT `outputs.step-N` 입력 해소 — 하류가 하드코딩 경로 대신 SOT Read), P3(Tier-II 증분 스트리밍). 이로써 "검증된 사실 척추가 단계마다 메모리를 통과"가 완성된다. **120 factory tests green**(P0a/P0b/P0 +10, P1 +4).
