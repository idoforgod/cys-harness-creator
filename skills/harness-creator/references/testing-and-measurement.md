...
6. [테스트 프롬프트 & assertion 작성법 (보존된 원본 지혜)](#6-테스트-프롬프트--assertion-작성법-보존된-원본-지혜)
7. [블라인드 독립 채점자 규율 (자기채점 금지)](#7-블라인드-독립-채점자-규율-자기채점-금지)
8. [Description 트리거 검증 — near-miss 중심](#8-description-트리거-검증--near-miss-중심)
9. [반복 개선 루프 & 진화 피드백](#9-반복-개선-루프--진화-피드백)
10. [워크스페이스 & provenance 구조](#10-워크스페이스--provenance-구조)
11. [원본 prose → CYS 게이트 대응표](#11-원본-prose--cys-게이트-대응표)
...
| **Gate 1 — 정적 빌드** | `validate_harness.py` | Phase 5 | 구조·계약·티어·게놈 머신체크 세트 | `exit 0` (error 0건) |
...
Phase -1   Gate 0  warrant.classify    → 하네스 만들 가치 있나?
   ↓
Phase 1-4  (graph.json + agents + schemas + emit)
   ↓
...
> **정성 vs 정량의 재배치.** 원본은 "정성적 평가(사람 리뷰) + 정량적 평가(assertion 자동채점)"의 조합이라 했다. CYS에서 *정량 채점은 전부 게이트로 자동화*되고, *정성 판단은 채점자를 모델 에이전트로 격상*시켜 블라인드로 만든다(§7). 즉 "사람이 본다"는 측정 경로에서 빠지고, 사람은 게이트 결과를 보고 **개선 의사결정**을 내린다.
...
graph.json이 완성되면 실행 전에 토큰 비용을 추정한다. 이것이 CYS의 **dry-run**이다 — 실제 agent()를 spawn하지 않고 그래프만 읽어 비용을 낸다.
...
- `weighted_units = Σ est_tokens × TIER_COST_WEIGHT`(haiku 1 / sonnet 3 / opus 5) → band `LOW`(<5e5) / `MEDIUM`(<5e6) / `HIGH`.
...
2. `approval_required=true`면 밴드를 **표시하고 사용자의 명시적 "approve"를 기다린다**. 첫 agent()가 spawn되기 전에 BLOCK한다.
3. 실제 실행 시 `budget.total`이 워크플로우 런타임의 **하드 ceiling**이다 — cost-band 추정과 별개로 런타임이 초과를 강제 차단한다(`ensure(min)` 가드).
...
| `TIER_MISMATCH` | error | node.model == agent frontmatter model | — |
...
| `DOC_DRIFT` | error | README phase-count == 오케스트레이터 SKILL phase-count | "문서 일치" |
| `W1_GENOME` | error | harness.md에 inherited-DNA 마커(AC-1~3) | — (게놈 신규) |
| `GENOME_PRESENT` | error | 9개 load-bearing 게놈 파일 전수 이식 | — (게놈 신규) |
| `RUNTIME_DECLARED` | error | `.harness/RUNTIME.json` canonical=cys-mode-a | — (2런타임 명시) |
| `HOOK_REGISTERED` | error | settings.json이 보안 hook 4종 wiring | — (보안 신규) |

\* `RATIONALE_MISSING` level은 `constants.json`의 `MODEL_RATIONALE_MISSING`(기본 `warn`)이 결정한다.

### 3-2. 모델 티어 정책 V1/V2/V3 — "all agents use opus" 폐기의 실행화

원본의 흔한 안티패턴 "모든 에이전트를 opus로"를 CYS는 **머신으로 거부**한다. `model-tier-policy.js`의 role→tier가 SoT이고, validator가 이를 미러링한다:
...
> **왜 이게 "테스트"인가.** 원본은 "에이전트를 분할하는 기준(전문성/병렬성/컨텍스트/재사용)"을 잘 정리했지만, 분할 *후* 각 에이전트가 적정 모델을 쓰는지는 사람 검토에 맡겼다. CYS는 role-class를 id/agent 이름에서 정규식으로 추론해(`_base_role_class`) 티어 위반을 자동 검출한다. 즉 *비용/품질 트레이드오프가 빌드 게이트로 강제된다.*

### 3-3. 게놈 무결성 — 상속이 깨지지 않았는지

모든 자식 하네스는 AgenticWorkflow 게놈(228파일, 자족)을 `inherit_genome.py`로 전수 상속한다. validator는 이 상속이 *부분*이 아니라 *전수*임을 강제한다:

- `GENOME_PRESENT`: `_context_lib.py`, `context_guard.py`, `block_destructive_commands.py`, `output_secret_filter.py`, `security_sensitive_file_guard.py`, `soul.md`, `AGENTS.md`, `CLAUDE.md`, `.harness/GENOME.json` — 이 9개가 없으면 error.
- `RUNTIME_DECLARED`: 두 상속 런타임(canonical=`cys-mode-a` workflow.js + 대안 `awf-prompt-runner`)이 RUNTIME.json에 disambiguate되어야 한다.
- `HOOK_REGISTERED`: settings.json의 hooks가 보안 4종을 wiring해야 한다.

> 이 체크들이 보장하는 것: 자식 하네스는 **이미 풍부한 운영 머신(컨텍스트 보존 hook·4계층 품질 게이트·보안 hook)을 가지고 태어난다.** 그래서 이 문서가 안내하는 테스트는 상속된 back-half가 아니라 **front-half 설계(domain→graph.json+agents+schemas)의 정확성**에 집중한다.
...
4. warn만 있으면 (exit 2) → 진행 가능하되 RATIONALE_MISSING 등은 보강 권장
```
...
**계약: 스킬은 no-skill baseline을 측정 가능하게 능가할 때만 등록된다.**
...
### 4-1. 두 단계 (probe 생성 → 채점)

`lift_gate.py`는 순수·결정론이라 agent를 직접 spawn하지 않는다(워크플로우 런타임만 가능). 그래서 책임을 둘로 나눈다:

**① emit-probe** — 스킬 스펙에서 `lift_probe.workflow.js` 템플릿을 생성:
```bash
...
**② 런타임이 probe 실행** → with/without results를 PRODUCE:
- with_skill arm: **sonnet** + 스킬 가이드를 프롬프트에 inline(워크플로우 agent()에 `skill:` 옵션이 없으므로 PROMPT INLINING).
- baseline arm: **haiku**, 스킬 없음.
- 두 arm은 *답만 생산한다 — 자기채점 금지.*
- 별도의 **블라인드 opus 채점자**가 두 답(A=with-skill, B=baseline)을 채점한다. 채점자는 어느 쪽이 스킬인지 모른다.

**③ score** — 런타임이 낸 results.json을 결정론적으로 채점:
```bash
...
> **핵심 통찰(M1, 경험적).** 초기 self-check 버전에서는 각 arm이 *자기 출력을 자기가 채점*했다 → haiku baseline이 자기 점수를 all-pass로 부풀려 lift가 0으로 붕괴, 게이트가 아무것도 측정하지 못했다. 수정: 두 arm은 답만 생산하고, **분리된 블라인드 opus 채점자**가 둘 다 채점한다(어느 쪽이 스킬인지 모름). 이것이 §7의 독립채점자 규율이고, 헤드투헤드 suite와 동일한 원칙이다.
...
원본의 "Comparator(블라인드 비교자)"는 *선택적 고급 기능*이었고 1회 비교였다. CYS는 이를 **n-run 중앙값 + provenance가 박힌 정식 suite**로 격상한다. lift_gate(Gate 2)가 "등록 자격"을 본다면, h2h(Gate 3)는 "CYS 하네스가 no-harness baseline을 *엄밀하게* 이기는가"를 입증한다.

### 5-1. suite 구조 (h2h_suite.workflow.js)
...
산출: `{ runs:[{run, c2_pass_rate, c3_pass_rate, a_is, b_is, ...}], provenance }`. runs[] 배열을 aggregator에 넘긴다.
...
aggregate 출력 note가 강제하는 것: *"INCONCLUSIVE면 마진을 못 넘은 것; variance가 높으면 n을 늘려라; CYS가 이기지 못하는 도메인은 정직하게 보고하라."* scorecard(`deep-research.scorecard.json`)도 동일: *"Report domains where CYS does NOT win honestly."*
...
## 6. 테스트 프롬프트 & assertion 작성법 (보존된 원본 지혜)
...
### 6-2. Assertion 작성 — discriminating해야 한다
...
> **QA 경계 교차비교 + 7개 실버그 패턴(보존).** 채점자가 산출물에서 검증가능한 claim을 추출해 교차검증하는 원본 규율은 그대로 유효하다 — CYS에서는 이것이 채점자 에이전트(§7)의 일이다. QA 노드(qa-scan, haiku)를 하네스에 두면 인접 노드의 출력 경계를 교차비교해 흔한 버그 패턴(누락 인용, 잘못된 조인, 침묵 해소된 충돌, 단위 불일치 등)을 잡는다. 이 패턴 카탈로그는 assertion 설계의 원천이다.

---

## 7. 블라인드 독립 채점자 규율 (자기채점 금지)
...
1. **피드백을 일반화하라** — 테스트 예시에만 맞는 좁은 수정은 오버피팅. 원리 수준에서 수정. (CYS에서는 graph.json/agent 본문을 고치는 것이지 특정 입력에 분기 추가가 아니다.)
2. **무게를 벌지 않는 것은 제거하라** — 트랜스크립트를 읽고 비생산적 작업을 시키는 부분을 삭제. (TIER_OVERSPEND warn이 나는 노드를 다운티어하는 것도 여기 포함.)
...
### 9-2. 반복 절차 (CYS 게이트 통합)

```
1. graph.json 또는 agent/schema 수정
2. validate_harness.py 재실행 (Gate 1 PASS 확인)        ← 구조 회귀 즉시 검출
3. warrant --graph 로 cost-band 재확인                  ← 비용 회귀 검출
4. (스킬 변경 시) lift_gate 재측정 (Gate 2)             ← 등록 자격 유지 확인
5. (엄밀히) h2h_suite n-run 재집계, 이전 verdict와 비교  ← 우위 회귀 검출
6. 결과 비교 → 다시 수정 → 반복
```

**종료 조건:** Gate 1 PASS + Gate 2 register + (필요시) Gate 3 CYS-WINS, 그리고 의미 있는 개선이 더 없을 때.

### 9-3. 초안 → 재검토 패턴 (보존)

agent 본문·schema·description을 한 번에 완벽히 쓰려 하지 말고 초안을 쓴 뒤 **새로운 시각으로 다시 읽고** 개선한다.

### 9-4. provenance 기반 회귀 추적 (CYS 신규)

git repo가 rollback substrate다(Phase 7). h2h_aggregate의 provenance(git_sha + harness_version + model_id)를 매 측정에 스탬프하면, *어느 커밋·어느 모델에서 우위가 바뀌었는지*를 정확히 추적할 수 있다. 원본의 "iteration-N 디렉토리 보존"을 git history + provenance로 대체한 것이다.

---

## 10. 워크스페이스 & provenance 구조

CYS 하네스의 테스트/측정 산출물은 하네스 repo 안에 산다(원본의 별도 `_workspace/`와 달리 자식 repo의 일부):

```
<TARGET>/                          # 생성된 하네스 (git repo = rollback substrate)
├── .harness/
│   ├── graph.json                 # 계약 (단일 writer = 메타스킬)
│   ├── workflow.js                # emit된 Mode A 런타임 (수정금지)
│   ├── predicates.json            # warrant Phase -1 입력
│   ├── RUNTIME.json               # canonical=cys-mode-a
│   └── harness.lock               # write_paths → node 맵
├── schemas/*.json                 # output 계약 (workflow.js S-table와 일치)
├── evals/
...
**규칙:**
- scorecard/runs 파일은 **서술적 도메인 이름** 사용(예: `deep-research.scorecard.json`).
- probe·workflow.js는 자동생성 — **손으로 고치지 않는다**(헤더 경고 준수). 고칠 게 있으면 graph.json/skill.json을 고치고 재emit.
...
> **한 줄 요약.** 원본은 *"어떻게 테스트해야 하는가"*를 잘 가르쳤다. CYS는 그 가르침을 *"테스트하지 않으면 빌드가 실패하고, baseline을 못 이기면 등록이 거부되며, 비용 승인 없이는 실행이 막힌다"*로 강제한다. 측정이 권고에서 계약으로 바뀐 것 — 그것이 CYS 하네스가 idoforgod/harness 대비 갖는 우위의 핵심이다.
