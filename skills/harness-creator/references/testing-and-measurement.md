...
| `MEASUREMENT_DRIFT` | error | evals verdict가 없는데 README/SKILL이 `CYS-WINS` 광고 | — (정직성 신규) |
...
원본의 흔한 안티패턴 "모든 에이전트를 opus로"를 CYS는 **머신으로 거부**한다. `model-tier-policy.js`의 role→tier가 SoT이고, validator가 이를 미러링한다(`TIER_BY_ROLE_CLASS`):
...
> ⚠️ 이전 판본의 "median(C2)=0.875 / +37.5pp / CYS-WINS"는 **hand-authored HYPOTHESIS fixture**였고 실측과 모순되어 **폐기**됐다. 모든 h2h/lift 인용 수치는 `evals/*.verdict.json`(디스크)과 일치해야 하며, `validate_harness.py`의 `MEASUREMENT_DRIFT` 체크가 이를 강제한다(verdict가 없는데 doc이 `CYS-WINS`를 광고하면 빌드 실패).
...
| "측정 안 했으면 이겼다 말하지 마라" | `MEASUREMENT_DRIFT` (verdict 없으면 CYS-WINS 금지) | `exit 1` |
