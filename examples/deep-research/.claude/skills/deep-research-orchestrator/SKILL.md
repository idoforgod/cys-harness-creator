...
- **gather** (`researcher`, mech=single): `Agent(subagent_type="researcher", model="haiku")` — 입력=Phase 1 입력(query), 반환=JSON(schemas/findings.json 스키마 준수).
- **fetch** (`fetcher`, mech=single): `Agent(subagent_type="fetcher", model="haiku")` — 입력='gather' 노드 출력, 반환=JSON(schemas/findings.json 스키마 준수).
- **verify** (`verifier`, mech=reflect-then-revise): `Agent(subagent_type="verifier", model="sonnet")` — 입력='fetch' 노드 출력, 반환=JSON(schemas/findings.json 스키마 준수).
  - critic(`opus`) Agent가 적대적 비평 → reviser가 수정, max_rounds=2, approved=true면 조기 종료.
- **synthesize** (`synthesizer`, mech=single): `Agent(subagent_type="synthesizer", model="opus")` — 입력='verify' 노드 출력, 반환=JSON(schemas/report.json 스키마 준수).
