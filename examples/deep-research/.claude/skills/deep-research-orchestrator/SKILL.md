...
3. `budget.max_spawns`는 emit이 graph에서 **결정론적으로 계산한 7**로 고정한다(LLM이 다시 합산하지 않음 — `sot_init.estimate_max_spawns`가 cold-start에 같은 값을 시드). 런타임 `budget_block.py`(PreToolUse)가 이 천장 초과 시 exit-2 차단.
...
- **교차-실행 도메인 메모리 (Tier II, `.harness/memory/`)** — 이 하네스가 *반복 실행*을 거쳐 도메인 지식을
  ...
  - **작업 시작 시 회상**: `Grep "deep research" .harness/memory/runs/index.jsonl` 로 과거 유사 실행을
