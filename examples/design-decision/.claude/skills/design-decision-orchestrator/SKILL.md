...
description: "design-decision 하네스를 Claude Code 프리미티브(Agent/TeamCreate)로 실행하는 오케스트레이터. 'design-decision' 관련 작업·생성·분석 요청 시 사용. 후속: 다시 실행, 재실행, 업데이트, 수정, 보완, 'design-decision의 일부만 다시', 이전 결과 기반 개선 요청 시에도 반드시 이 스킬을 사용."
---
...
|---|---|---|---|---|---|
| propose | proposer | opus | single | Read, Write, Glob, Grep | _workspace/01_propose/design.json |
| adjudicate | debater | sonnet | debate-with-judge | Read, Glob, Grep, WebSearch | _workspace/02_adjudicate/verdict.json |

## 워크플로우

### Phase 0: 장기기억 회상 + 컨텍스트 + SOT 초기화
1. `<harness>/` 존재로 분기: 초기 / 재실행 / 부분 재실행 / 마이그레이션.
2. **장기기억 회상 (Tier II — 작업 입력, 실행 단계·MANDATORY)**: graph 실행 *전에* 과거 도메인 지식을 회상한다 — `Grep "design decision" .harness/memory/runs/index.jsonl`로 유사 run을 찾아 **매치된 run만** `Read .harness/memory/runs/<run_id>/`(`design decision`는 emit이 `query_norm(harness_name)`로 **결정론적으로 baking**한 회상 키 — LLM이 토큰을 새로 추정하지 않으며 쓰기 키와 글자 단위로 일치) + `Read .harness/memory/domain-knowledge.yaml`(+ `risk/decisions.jsonl`로 금기 확인) → 회상한 엔티티·제약을 `_workspace/_recall.json`에 떨궈 **하류 에이전트의 작업 입력**으로 넘기고, `state.yaml`의 `audit_log`에 `{"recall": "hit|miss", "injected": [...]}`를 기록한다. 무매치/콜드면 `_workspace/_recall.json = {"cold": true}`로 진행(선례 없음). 회상 직후 **run START**로 `.harness/memory/runs/index.jsonl`에 `{"run_id": <id>, "status": "in_progress", "query_norm": "design decision"}` 1줄을 append한다(`query_norm`은 위 회상 키와 동일 리터럴 — 읽기/쓰기 키 동치 보장) — Tier-II 쓰기는 종단 일괄이 아니라 **증분 스트리밍**이라 크래시·재개·병렬 run이 부분 진행을 회상한다(완료 시 Phase 3에서 같은 레코드를 `status: "completed"`로 갱신). 이는 아래 "메모리 운영" 섹션의 prose가 아니라 **Phase 0의 실행 단계**다 — 회상→주입 도관의 입력 절반(맹신 금지: 회상은 현재 요건에 맞게 검증 후 사용).
3. `.harness/state.yaml`(SOT)을 작성/갱신한다 — **오케스트레이터 단독 쓰기**. 필드:
   `current_step, outputs{}, budget{spawns_used:0, max_spawns:5}, pacs{}, audit_log[]`.
...
3. `budget.max_spawns`는 emit이 graph에서 **결정론적으로 계산한 5**로 고정한다(LLM이 다시 합산하지 않음 — `sot_init.estimate_max_spawns`가 cold-start에 같은 값을 시드). 런타임 `budget_block.py`(PreToolUse)가 이 천장 초과 시 exit-2 차단.
...
1. **`TeamCreate(team_name="design-decision-team", members=[...])`** — 멤버(각 노드 agent; frontmatter model·tools 런타임 강제):
   - `proposer` (model=opus, tools=Read, Write, Glob, Grep) — 'propose' 노드
   - `debater` (model=sonnet, tools=Read, Glob, Grep, WebSearch) — 'adjudicate' 노드
...
2. **`TaskCreate`** — 각 노드를 task로 생성, 의존성은 graph edges(toposort 보존):
   - `TaskCreate(subject="propose", owner="@proposer")`
   - `TaskCreate(subject="adjudicate", owner="@debater", depends_on=["propose"])`
...
3. (선택) head-to-head: `evals/design-decision.scorecard.json` 기준 C2(이 하네스) vs C3(no-harness) n≥5 → `h2h_aggregate.py`.
...
- **교차-실행 도메인 메모리 (Tier II, `.harness/memory/`)** — 이 하네스가 *반복 실행*을 거쳐 도메인 지식을
  ...
  - **작업 시작 시 회상**: `Grep "design decision" .harness/memory/runs/index.jsonl` 로 과거 유사 실행을
