...
## 실행 모드: agent (기본=agent 순차 sub-spawn; team=TeamCreate/SendMessage 실제 emit; hybrid=단계별 혼합)
...
**실행 모드: agent** — toposort 순서로 노드를 spawn하고(병렬 fan-out은 `run_in_background`), 각 노드 후 품질 게이트를 통과시킨다. spawns_used를 spawn마다 +1(단일쓰기, PostToolUse `spawn_counter`).

- **propose** (`proposer`, mech=single): `Agent(subagent_type="proposer", model="opus")` — 입력=Phase 1 입력(query), 반환=JSON(schemas/design.json 스키마 준수).
- **adjudicate** (`debater`, mech=debate-with-judge): `Agent(subagent_type="debater", model="sonnet")` — 입력='propose' 노드 출력, 반환=JSON(schemas/verdict.json 스키마 준수).
  - 2명 debater를 2라운드 토론 후 judge(`opus`) Agent가 판정.
