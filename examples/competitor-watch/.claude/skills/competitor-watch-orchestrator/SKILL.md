...
- **gather** (`scout`, mech=single): `Agent(subagent_type="scout", model="haiku")` — 입력=Phase 1 입력(query), 반환=JSON(schemas/items.json 스키마 준수).
- **dedupe** (`deduper`, mech=single): `Agent(subagent_type="deduper", model="haiku")` — 입력='gather' 노드 출력, 반환=JSON(schemas/items.json 스키마 준수).
- **brief** (`briefer`, mech=single): `Agent(subagent_type="briefer", model="opus")` — 입력='dedupe' 노드 출력, 반환=JSON(schemas/brief.json 스키마 준수).
